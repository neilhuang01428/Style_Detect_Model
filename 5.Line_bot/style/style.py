...
real_classesFile = "./config/cfg/deepfashion0827.names" #deepfashion0827.names
classesFile2 = "./config/cfg/chinese_label.names" #deepfashion0827.names  chinese_label.names
...
def draw_rectangle(frame,label, center_x, center_y, width, height):
    ...
    cv2.putText(frame, label, (left, top-10), cv2.FONT_HERSHEY_COMPLEX, fontSize, labelColor, fontBold)
...
def sty_clf(label,pro,color,clf="Best"):
    detect_list = [ 0 for i in range(31)] #21 label + 10 color
    # print(len(detect_list)) range(len(classes2))
    ...
    for i in range(len(color_name)):
        detect_list[i+21] = color[color_name[i]]
#        total_col_name.append(color_name[i])

    detect_list = pd.DataFrame(detect_list,index=total_col_name).T
    
    if clf =="Best":
        with open('./config/pickle/final_label/best_xgb_Base model.pickle', 'rb') as file:
...
def predict(img_path):
    ...
    person_boxs = postprocess(frame, outs, orgFrame,classifier="person")
    object_boxs = postprocess(frame, outs2, orgFrame,classifier="object")
    ...
    if person_boxs==[] and object_boxs==[]: #沒有人 也沒有物件的狀況
        ...
        for p in person_boxs:
            p_num += 1
            draw_rectangle(orgFrame,"Person:%s"%p_num,p[2],p[3],p[4],p[5])
        target_name = img_path.split("/")[-1]
        cv2.imwrite(static_processed_path+"/"+target_name,orgFrame)
        resp["pic_name"] = target_name
        ...
        #Draw rectangle
        x_temp = (left_min + right_max)/2
        y_temp = (top_min +bottom_max)/2
        w_temp = right_max -left_min
        h_temp = bottom_max -top_min
        draw_rectangle(orgFrame,"Detect Range",x_temp,y_temp,w_temp,h_temp)
        target_name = img_path.split("/")[-1]
        cv2.imwrite(static_processed_path+"/"+target_name,orgFrame)
        resp["pic_name"] = target_name
        ...
        clf_type ="Best"
        style_type ,style_proba = sty_clf(label,pro,color_temp,clf=clf_type)
...
#                    object_posi.append([o[2],o[3],o[4],o[5]])
            ...
            if posses_boxs!=[]:
                ...
                w_temp = right_max -left_min
                h_temp = bottom_max -top_min
                draw_rectangle(orgFrame,"Person:%s"%(p_num),x_temp,y_temp,w_temp,h_temp)
            
            else:
                poor_people+=1
                resp["color"].append(None)
...
#                resp["object_prob"].append(None)
        ...
        if poor_people<len(person_boxs):#有人有東西
            target_name = img_path.split("/")[-1]
            cv2.imwrite(static_processed_path+"/"+target_name,orgFrame)
            resp["pic_name"] = target_name
        else:#所有人都沒有東西
